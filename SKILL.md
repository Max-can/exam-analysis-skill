...
- **有答案/错题数据**：包括正确答案、学生作答、得分率、错误人数、班级成绩等任意一项 → 进入**增强模式**
- **无答案/错题数据**：只有知识点和难度信息 → 进入**标准模式**

两种模式都要在第三步补全无法从数据中直接获取的内容，不要因为数据不完整就停下来。

### 第三步（标准模式）：智能补全 + 生成标准五模块报告
...
按固定顺序输出**标准五模块报告**，然后导出PDF（见下方"导出PDF"说明）。

### 第三步（增强模式）：基于真实数据深度分析 + 生成增强七模块报告

利用真实的答案和错题数据做更精准的分析：

- **错因分析**：不再推断，直接从学生实际错误中归纳（如"选B的学生占40%，混淆了XX概念"）
- **失分预警**：按实际得分率从低到高排序，真实反映高频失分题
- **得分分布**：如有逐人成绩，计算平均分、各分段人数、最高/最低分
- **共性错误**：跨题目归纳同一类错误的反复出现（如多题都有计算失误）

按固定顺序输出**增强七模块报告**，然后导出PDF（见下方"导出PDF"说明）。
...
在标准五模块基础上，将**模块四**和**模块五**升级，并新增**模块六**和**模块七**，共七模块：
...
| 题号 | 知识点 | 得分率 | 主要失分原因 |
|------|--------|--------|-------------|
| [题号] | [知识点] | [x%] | [基于实际错误数据的真实原因] |
| ... | | | |

**共性错误归纳：**（跨多题反复出现的错误类型）
- [错误类型1]：涉及第[题号]题，表现为[具体描述]
- [错误类型2]：...
...
| 题号 | 考察知识点 | 难度 | 得分率 | 正确答案 | 常见错误答案 | 错因分析 | 解决方案 |
|------|-----------|------|--------|---------|-------------|---------|---------|
| 1 | [知识点] | 易/中/难 | [x%] | [答案] | [错误选项及占比] | [基于真实错误的具体分析] | [具体建议] |
| ... | | | | | | | |

若数据中没有错误答案明细，只有得分率，则省略"常见错误答案"列，保留其余列。

---

### 六、班级得分分布

> 仅当用户提供了逐人成绩时输出此模块，否则跳过。
...
列出得分率低于50%的学生（或成绩后15%），标注其主要失分板块，便于教师重点辅导。
...
- 逐题分析表格（标准模式）col_widths 参考：`[12, 28, 12, 60, 58]*mm`，字号用9pt
- 逐题分析表格（增强模式，含得分率和错误答案列）col_widths 参考：`[10, 26, 10, 14, 20, 28, 40, 42]*mm`，字号用8pt
- 增强模式的模块六、七仅在有逐人成绩时生成，否则跳过
